...
def render_text(summary: dict[str, Any], color: bool | None = None) -> str:
    ...
    lines = [
        f"{orange}KX-DEFENDER REPORT{reset}  {muted}·{reset}  "
        f"window={accent}{hours}h{reset}  "
        f"generated={muted}{generated}{reset}",
        f"total alerts: {accent}{total}{reset}",
        "",
    ]

    by_sev = summary.get("by_severity") or {}
    if by_sev:
        lines.append(f"{orange}BY SEVERITY{reset}")
    ...
    if by_mod:
        lines.append(f"{orange}BY MODULE{reset}")
    ...
    if top:
        lines.append(f"{orange}TOP EVENTS{reset}")
        for entry in top:
    ...
    if per_hour:
        ...
        lines.append(f"{orange}PEAK HOUR{reset}  {muted}(UTC){reset}")
        lines.append(f"  {accent}{peak_key}{reset}  → {peak_n} alerts")
        lines.append("")

    latest = summary.get("latest_per_severity") or {}
    if latest:
        lines.append(f"{orange}LATEST PER SEVERITY{reset}")
        for sev in SEVERITY_ORDER:
