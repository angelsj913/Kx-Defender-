...
**작성일**: 2026-07-30  
**상태**: DRAFT → 구현 진행  
**담당**: Cursor agent — **명령어 / 시스템 파일만** (디자인·비주얼은 Claude)  
**충돌 회피**: Claude PR 경로(`docs/API.md`, `PRD-KX-DEFENDER-V2.md`, `src/core/defense/*` 등)와 **겹치지 않음**  
**관련 증상 (사용자 제보)**: Windows PowerShell eDEX HUD에서 한글 깨짐, `kx` 동사 실행 실패/불친절, 불필요 HUD 문구
...
시각 테마(색, 패널 레이아웃, 로고 아트)는 Claude 디자인 트랙에 맡긴다.
...
| P5 | HUD 상단 `Kx-DEFENDER · eDEX HUD · TRON LINK`, 하단 `명령 입력 · lang…` | 장식 문구가 명령 UX를 가림 |
...
5. Claude 디자인 PR과 **파일 충돌 0**.

### Non-Goals
- eDEX 패널 레이아웃/색상/로고 리디자인 (Claude)
- 외부 보안 툴 연동
...
| 트랙 | 담당 | 파일 예 |
|---|---|---|
| **Commands / System** (본 PRD) | Cursor | `scripts/npx-entry.js`, `scripts/kx-update.js`, `scripts/edex-shell.js`(동작만), `scripts/npm-setup.js`, `Install-Kx.ps1`, `services/orchestrator/kx_defender/*`, `fixtures/catalog/kxlang_lexicon.json`, `docs/prd/kx-cli-stability.md` |
| **Design / Visual** | Claude | Console CSS/HTML 비주얼, FigJam/Figma, 패널 아트 — **명령 문법·인코딩·파서 금지** |
...
2. Node spawn (`edex-shell` / `runKx`) env에 위 변수 주입; Windows에서 가능하면 세션 `chcp 65001` (Install-Kx / HUD 기동 시 1회).
...
**삭제 (사용자 요청)**
- 상단: `Kx-DEFENDER  ·  eDEX HUD  ·  TRON LINK`
...
3. Claude 디자인 트랙과 HUD 프레임 API 계약 (텍스트 슬롯만 시스템 소유)
...
| Claude가 같은 HUD 파일 수정 | 텍스트 상수만 시스템 소유 / 디자인 충돌 시 Claude는 CSS·레이아웃만 |

---

## 9. Out of scope file list (do not touch — Claude / other)
